...
![d](/posts/wafer-map_files/Figure_1-2.png)
...
require(dplyr)
#df <- df %>% mutate(ROW = factor(ROW), ROW = factor(ROW, levels = rev(levels(ROW))))
ggplot(df,aes(x=COL,y=ROW,fill=DATA)) + geom_tile() + scale_fill_gradientn(colours = jet.colors(7)) +scale_y_reverse()
#ggplot(df,aes(x=COL,y=reorder(ROW,desc(ROW)),fill=DATA)) + geom_tile() + scale_fill_gradientn(colours = jet.colors(7))
...
```{r}
library(pheatmap)
#mat <- rpivotTable(df,rows = ~ROW,cols=~COL,vals=~DATA)
#mat <- melt(df,id=c('COL','ROW'))
mat <- as.matrix(spread(df,COL,DATA))
jet.colors <- colorRampPalette(c("#00007F", "blue", "#007FFF", "cyan", "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"))
breaksList = seq(60, 90, by =4)
pheatmap(mat,show_colnames = FALSE,show_rownames = FALSE,na_col = 'white',cluster_rows = FALSE,cluster_cols = FALSE,color=jet.colors(7),breaks=breaksList)
#mat <- melt(df,id=c('COL','ROW'))
